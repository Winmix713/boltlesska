...
function parseArgs(args) {
  const options = { version: 'baseline-v1', publish: false, selfTest: false, snapshot: null };
...
The default mode only reads and validates. --publish requires SUPABASE_URL and
...
async function main() {
  ...
  const { data: integrity, error: integrityError } = await admin.rpc('winmix_validate_data_version', { p_data_version_id: version.id });
  ...
  const sourceRows = await fetchAllMatches(admin, version.id);
  if (sourceRows.length !== version.match_count) throw new Error(`Read ${sourceRows.length} source rows; expected ${version.match_count}.`);
  ...
  const counts = validateOutput({ sourceIds: new Set(sourceRows.map((row) => row.id)), features, predictions, teamSnapshots, calibrationRows, expected: version.match_count });
  const durationMs = Date.now() - started;
  const report = { mode: options.publish ? 'publish' : 'dry-run', version: options.version, dataVersionId: version.id, parameterSnapshotId: snapshot.id, inputFingerprint: `${version.content_fingerprint}:${snapshot.parameters_fingerprint}`, durationMs, ...counts, generatedAt: new Date().toISOString() };
